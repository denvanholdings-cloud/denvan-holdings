    ...
    return (
      <div style={{ padding: "120px 0", textAlign: "center" }}>
        <div className="container">
          <h1 style={{ fontSize: "2rem", color: "#1B5E37", marginBottom: 16 }}>Article Not Found</h1>
            ...
            <article style={styles.articleContent}>
              {post.content.map((block, i) => {
                ...
                if (block.startsWith("**") && block.endsWith("**")) {
                  return (
                    <p key={i} style={{ ...styles.paragraph, fontWeight: 600, color: "#0D2818" }}>
                      {block.replace(/\*\*/g, "")}
                    </p>
                  );
                }
                  ...
                  <p key={i} style={styles.paragraph}>
                    {parts.map((part, j) => {
                      if (part.startsWith("**") && part.endsWith("**")) {
                        return <strong key={j} style={{ color: "#0D2818" }}>{part.replace(/\*\*/g, "")}</strong>;
                      }
              ...
              <div style={styles.articleCta}>
                <h3 style={{ fontSize: 20, fontWeight: 600, color: "#1B5E37", marginBottom: 8 }}>
                  Want to learn more?
                </h3>
              ...
              <Link key={p.slug} href={`/blog/${p.slug}`} style={{ textDecoration: "none" }}>
                <article className="card" style={{ padding: 0, overflow: "hidden" }}>
                  <div style={{
                    ...
                    <span style={{ fontSize: 32, fontWeight: 700, color: "#B8942A", opacity: 0.3 }}>DH</span>
                  </div>
                  <div style={{ padding: 24 }}>
                    <div style={{ display: "inline-block", padding: "3px 10px", background: "#F1F5F9", color: "#1B5E37", borderRadius: 100, fontSize: 11, fontWeight: 600, marginBottom: 10 }}>
                      {p.category}
                    </div>
                    <h3 style={{ fontSize: 16, fontWeight: 600, color: "#1B5E37", marginBottom: 8, lineHeight: 1.4 }}>
                      {p.title}
                    </h3>
